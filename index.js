...
async function start(data) {
    ...
    do {
        ...
        if (interaction.userInput === "staff") {
            console.log(chalk.green("Hello! You approach the staff room. Please feel free to ask any question.\n"));
        }
            ...
            if (value) {
                console.log("\nHello", chalk.italic.blueBright(`${answer.user}`), "! Welcome back to the class.\n");
                console.log(chalk.magenta.underline("Existing Students:"));
                console.log(data.stdArray.map((i) => i.name), "\n");
            }
            else {
                let newStudent = new Student(answer.user);
                data.addStudent(newStudent);
                console.log(chalk.red("\nNew Student Added!"));
                console.log("Hello", chalk.italic.blueBright(`${answer.user}`), "! Welcome to the class.\n");
                console.log(chalk.magenta.underline("Current Students:"));
                console.log(data.stdArray.map((i) => i.name), "\n");
            }
        }
        else {
            console.log(chalk.red("Thank you for visiting! Looking forward for the next time."));
            process.exit();
        }
    } while (true);
}
